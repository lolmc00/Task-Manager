...
class TodoItem(tooltip.ToolTipWidget):
	def __init__(self, parent:QtWidgets.QWidget=None, main:QtWidgets.QWidget=None, todo:task.Todo=None, grid:QtWidgets.QWidget=None):
		...
		self.label_title.setStyleSheet("font: 600 15px '나눔스퀘어'; %s" % ("text-decoration:line-through" if todo.isCompleted() else ""))
		...
		if self.todo.getDescription() != '':
			...
			self.edit_todo_description.setStyleSheet("font: 400 13px '나눔스퀘어'; border:0px")
		...
		self.btn_complete_redo.setFixedSize(80, 30)
		self.btn_complete_redo.clicked.connect(self.onClickRedoBtn if todo.isCompleted() else self.onClickCompleteBtn)
		self.btn_complete_redo.setStyleSheet("font:600 13px; border: 2px solid %s" % colors.COLOR_PURPLE)
		...
		self.btn_edit.setStyleSheet("font:600 13px; border: 2px solid %s" % colors.COLOR_AQUA)
